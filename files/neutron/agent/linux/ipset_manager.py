...
class IpsetManager(object):
    ...
    def set_members(self, id, ethertype, member_ips):
        ...
        LOG.info("cfarquhar: in set_members.  member_ips = {}  set_name = {}  add_ips = {}  del_ips = {}".format(member_ips, set_name, add_ips, del_ips))
    ...
    def set_members_mutate(self, set_name, ethertype, member_ips):
        ...
        if not self.set_name_exists(set_name):
            ...
            LOG.info("cfarquhar: in set_members_mutate, we found that set {} is unknown by manager.  Creating set with ethertype = {} and member_ips = {}".format(set_name, ethertype, member_ips))
            ...
            LOG.info("cfarquhar: in set_members_mutate.  Set {} was known by manager.  Let's calculate IPs to add/del".format(set_name))
            add_ips = self._get_new_set_ips(set_name, member_ips)
            del_ips = self._get_deleted_set_ips(set_name, member_ips)
            LOG.info("cfarquhar: IPs to add: {}".format(add_ips))
            LOG.info("cfarquhar: IPs to delete: {}".format(del_ips))
            if (len(add_ips) + len(del_ips) < IPSET_ADD_BULK_THRESHOLD):
                LOG.info("cfarquhar: IPs to change for set {} is below bulk threshold ({}).  Calling add/delete".format(set_name, IPSET_ADD_BULK_THRESHOLD))
                self._add_members_to_set(set_name, add_ips)
                self._del_members_from_set(set_name, del_ips)
            else:
                LOG.info("cfarquhar: IPs to change for set {} is above bulk threshold.  Calling refresh".format(set_name))
                self._refresh_set(set_name, member_ips, ethertype)
    ...
    def _add_member_to_set(self, set_name, member_ip):
        LOG.info("cfarquhar: entered _add_member_to_set")
        cmd = ['ipset', 'add', '-exist', set_name, member_ip]
        LOG.info("cfarquhar: cmd = {}".format(cmd))
        self._apply(cmd)
    ...
    def _del_member_from_set(self, set_name, member_ip):
        ...
        LOG.info("cfarquhar: entered _del_member_from_set.  cmd = {}".format(cmd))
    ...
    def _create_set(self, set_name, ethertype):
        ...
        LOG.info("cfarquhar: entered _create_set.  cmd = {}".format(cmd))       
        self._apply(cmd)
